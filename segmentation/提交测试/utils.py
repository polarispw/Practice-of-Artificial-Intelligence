...
def collate(folder):
    ...
    cla_list = [cls.split('-')[1].split('.')[0] for cls in res_list]
    cla = {'DCM': 0, 'HCM': 0, 'NOR': 0}
    for c in cla_list:
        cla[c] += 1
    most_pos = max(cla, key=cla.get)
    for res in res_list:
        if res.split('-')[1].split('.')[0] != most_pos:
            new_res = res.split('-')[0] + f"-{most_pos}.png"
            os.rename(os.path.join(folder, res), os.path.join(folder, new_res))
